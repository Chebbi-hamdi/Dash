...
const PreviewSelect: React.FC = () => {
  ...
  const [commentsAndPositions, setCommentsAndPositions] = useState<CommentData[]>([]);
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [showPopup, setShowPopup] = useState<boolean>(false);

  useEffect(() => {
    axios.get('http://localhost:3000/api/v0/comment/660d4204191a237afda1cf8d')
      .then((response) => {
        const { image, comments } = response.data;
        const commentsAndPositions: CommentData[] = comments.map((commentObj: any) => ({
  ...
  <div style={{ display: 'flex', justifyContent: 'center' }}>
    <div style={{ position: 'relative' }}>
      ...
      {commentsAndPositions.map(({ comment, position }, index) => (
        ...
        {hoveredIndex !== null && (
          <Popup
            comment={commentsAndPositions[hoveredIndex].comment}
            position={commentsAndPositions[hoveredIndex].position}
          />
        )}
      </div>
    </div>
