...
import { useGetAllSubSubTask } from '../../hooks/costumHooksTaskType';
interface Item {
  _id: string;
  title?: string;
  text?: string;
...
interface DropdownProps {
  ...
  const MultiSubSubTask: React.FC<DropdownProps> = ({ id , onSelectedChange }) => {
  ...
  const { data, isLoading } = useGetAllSubSubTask('SSTT', 'SSTT');
  ...
  const select = (dataId: string, event: React.MouseEvent) => {
    const index = options.findIndex((option) => option.value === dataId);
    if (index === -1) return; // Exit if no matching option is found
    ...
    if (!newOptions[index].selected) {
      ...
      if (selectedIndex !== -1) {
        ...
        setSelected(selected.filter((i) => i !== dataId));
      }
    }
...
export default MultiSubSubTask;
